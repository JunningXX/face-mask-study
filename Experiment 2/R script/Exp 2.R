...
study2$condition <-  factor(study2$condition, levels = c("unmask", "mask"))
study2$Sex <-  factor(study2$Sex)
...
study2$party <- factor(study2$party, levels = c("Conservative Party", "Labour Party", "Liberal Democrats", 
                                                ...
                                                "Prefer not to say"))
study2 <- study2 %>% mutate(protection_simple = ifelse(protection == "Only themselves" | protection == "Mostly themselves", "themselves",
                                                       ifelse(protection == "Equally themselves and other", 
                                                              "Equally themselves and other", "others"))) %>% 
  mutate(behaviour_simple = ifelse(behaviour == "Much less frequently" | behaviour == "Less frequently", 
                                   "less", "equally or more"))

study2$protection_simple <- as.factor(study2$protection_simple)
study2$behaviour_simple <- factor(study2$behaviour_simple, levels = c("equally or more", "less"))
...
Data_long <- cleandata %>% pivot_longer(cols = Responder3:Responder30, names_to = "Responder_value",
...
lm1 <- lm(Sender ~ condition + Sex + Order + Sex * condition , cleandata)
...
# Exploratory model 
lm5 <- lm(Sender ~ Sex + condition + Order + behaviour_simple + protection_simple + trustworthy +
...
Mask <- cleandata%>%filter(behaviour_simple=="less")
Unmask <- cleandata%>%filter(behaviour_simple!="less")
...
SF <- Data_long%>%filter(Order=="SenderFirst")
RF <- Data_long%>%filter(Order=="ResponderFirst")
cohensD(SF$Pound_returned,RF$Pound_returned)

model6 <- lme(fixed = Pound_returned~condition + Sex + Order + Sender + trustworthy + 
                protection_simple + behaviour_simple + condition*Sex, 
              random = ~Sender|ID, data = Data_long)
summary(model6)
...
cowplot::plot_grid(plot1, plot4,ncol=1,nrow = 2, rel_heights = c(1.6,1))
