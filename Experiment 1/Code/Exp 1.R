...
Data$Picture_label <- as.factor(Data$Picture.label)
...
Data <- Data %>% mutate(protection = ifelse(Protect_themselves_or_others == "Equally themselves and other",
                                             "Equally themselves and other", 
                                            ifelse(Protect_themselves_or_others == "Mostly others" | 
                                                     Protect_themselves_or_others =="Only others", "other","themselves")))
...
cleandata$Pic.Mask <- factor(cleandata$Pic.Mask, levels = (c('U', 'M')))
...
# Exploratory model
lm6 <- lm(Sender_Response ~  Pic.Mask + Sex + Pic.Gender + Order + Trustworthy + protection +
...
cohens_d(mask$Sender_Response, unmask$Sender_Response)

SF <- cleandata%>%filter(Order=="SenderFirst")
RF <- cleandata%>%filter(Order=="ResponderFirst")
cohensD(SF$Sender,RF$Sender)
...
Data_long <- cleandata %>% pivot_longer(cols = Responder_3:Responder_30, names_to = "Responder_value",
...
model4 <- lme(fixed = Pound_returned~Pic.Mask + Pic.Gender + Sex + Order + Sender_Response +
                Pic.Mask*Pic.Gender + Sex*Pic.Mask + Sex *Pic.Gender,
...
model6 <- lme(fixed = Pound_returned~Sex + Pic.Gender + Pic.Mask + Order +Sender_Response+ Trustworthy + 
                protection + Sex*Pic.Gender + Sex*Pic.Mask + Pic.Gender*Pic.Mask , 
              random = ~Sender_Response|ID, data = Data_long)
